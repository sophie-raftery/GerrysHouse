...
def run():
    ...
    # Exit door — invisible Door instance that leads back to Test_level
    exit_door = Door(
        pos           = (770, 140),
    ...
    # Make the door invisible by clearing its surface
    ...
    # Custom prompts to replace the Door default
    _exit_font    = pygame.font.SysFont(None, 20)
    _exit_open_s  = _exit_font.render("[E] Exit",         True, (255, 255, 255))
    _exit_open_sh = _exit_font.render("[E] Exit",         True, (0, 0, 0))
    _exit_lock_s  = _exit_font.render("Need vinyl + key", True, (255, 120, 120))
    _exit_lock_sh = _exit_font.render("Need vinyl + key", True, (0, 0, 0))
    ...
    while running:
                ...
                if event.key == pygame.K_e:
                    ...
                    # Exit door — needs vinyl + key
                        ...
                        if _can_exit(overlay.hotbar):
                            ...
                            pass  # prompt is shown via exit_door.show_prompt in draw
        ...
        make_bed.draw(display_surface)
        key_box.draw(display_surface)
        # Draw exit door: glow + custom locked/unlocked prompt
        if exit_door.show_prompt:
            ...
            lbl  = _exit_open_s  if unlocked else _exit_lock_s
            shad = _exit_open_sh if unlocked else _exit_lock_sh
